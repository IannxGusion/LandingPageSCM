          ...
          {filteredProduk.map((produk) => (
            ...
            >
              <div
                ...
                role="button"
                tabIndex={0}
                onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && openModal(produk.id)}
                aria-label={`Lihat ${produk.nama}`}
              >
                <motion.img
                  src={produk.gambar[0]}
                  alt={produk.nama}
              ...
              >
                ...
                <div className="flex items-center justify-between mt-3">
                  <div>
                    ...
                    <div className="text-xs text-neutral-500 dark:text-neutral-400">{produk.kategori}</div>
                  </div>
                  <div className="flex gap-1" aria-label={`Rating produk ${produk.rating} dari 5`}>
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        size={16}
          ...
          {isModalOpen && selectedProduct && (
            <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-describedby="modal-description">
              ...
              <motion.div
                ...
                className="relative bg-white dark:bg-neutral-900 rounded-3xl shadow-2xl overflow-hidden max-w-6xl w-[96%] md:w-5/6 lg:w-4/6"
              >
                <div className="flex items-start justify-between px-5 py-4 border-b dark:border-neutral-800">
                  <div>
                    <h3 id="modal-title" className="text-lg font-semibold">{selectedProduct.nama}</h3>
                    <div className="text-xs text-neutral-500 dark:text-neutral-400">{selectedProduct.kategori} • {selectedProduct.rating}★</div>
                  </div>
                  <button
                    type="button"
                    onClick={closeModal}
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 p-5">
                  <div className="md:col-span-7 relative">
                    <button
                      ...
                      className="absolute left-3 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-white/80 dark:bg-neutral-800/60 hover:scale-105 transition"
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-white/80 dark:bg-neutral-800/60 hover:scale-105 transition"
                    ...
                    <div className="rounded-xl overflow-hidden relative">
                      <motion.img
                        key={selectedProduct.gambar[activeImageIndex]}
                        src={selectedProduct.gambar[activeImageIndex]}
                        alt={`${selectedProduct.nama} - gambar ${activeImageIndex + 1}`}
                        ...
                        onError={onImgError}
                      />
                      <div className="absolute left-4 bottom-4 bg-black/40 text-white text-xs px-2 py-1 rounded-md">
                        Gambar {activeImageIndex + 1} / {selectedProduct.gambar.length}
                      </div>
                    </div>
                    ...
                    <div className="mt-3 flex items-center gap-2 overflow-x-auto py-2">
                      {selectedProduct.gambar.map((img, idx) => (
                        <button
                          ...
                          className={`flex-shrink-0 rounded-lg overflow-hidden border transition ${
                        ...
                        >
                          <img
                            src={img}
                            alt={`Thumbnail gambar ${idx + 1} dari ${selectedProduct.nama}`}
                  ...
                  <div className="md:col-span-5 flex flex-col gap-4">
                    <div className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{selectedProduct.harga}</div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center border rounded-lg overflow-hidden">
                        ...
                        <div className="px-4 py-2 min-w-[48px] text-center" aria-live="polite" aria-atomic="true">{qty}</div>
                        <button
                          type="button"
                      ...
                      <button
                        type="button"
                        onClick={() => addToCartAndGoToCart(selectedProduct, qty)}
                        className="flex-1 px-4 py-3 rounded-xl bg-indigo-600 text-white font-medium shadow hover:bg-indigo-700 transition"
                      >
